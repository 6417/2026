...
public class SwerveSubsystem extends SubsystemBase {
    ...
    public void periodic() {
        ...
        if (!driveIsAutomated) {
            runManualDrive(joystickAxes);
        }
    }

    private void runManualDrive(double[] joystickAxes) {
        double translationX = -joystickAxes[1];
        double translationY = -joystickAxes[0];

        if (Constants.SwerveSubsystem.oldTurnSystem && !intakeMode) {
            // Old turn mode: right stick X controls angular velocity directly.
            drive.drive(
                    SwerveMath.scaleTranslation(
                            new Translation2d(
                                    translationX * drive.getMaximumChassisVelocity(),
                                    translationY * drive.getMaximumChassisVelocity()),
                            0.8),
                    -joystickAxes[2] * drive.getMaximumChassisAngularVelocity(),
                    true,
                    false);
            return;
        }
        ...
        double headingY;
        if (intakeMode) {
            headingX = joystickAxes[0] * 100.0;
            headingY = joystickAxes[1] * 100.0;
        } else {
            int allianceSign = DriverStation.getAlliance().orElse(Alliance.Blue) == Alliance.Blue ? 1 : -1;
            headingX = allianceSign * -joystickAxes[2];
            headingY = allianceSign * -joystickAxes[3];
        }

        Translation2d scaledInputs = SwerveMath.scaleTranslation(new Translation2d(translationX, translationY), 0.8);
