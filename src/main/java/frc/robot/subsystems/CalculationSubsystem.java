...
public class CalculationSubsystem extends SubsystemBase {
    ...
    private void calculateMOVEMENT() {
        ...
        double rpm_to_meterspersec = 1.0 / 60.0 * Math.cos(Constants.Shooter.shooterAngle / 180.0 * Math.PI) * Math.PI
                * Constants.Shooter.shooterWheelDiameter_meters;
        double vball = rpm_average * rpm_to_meterspersec;
        Logger.recordOutput("Calculation/Vball", vball, Units.MetersPerSecond);
        Logger.recordOutput("Calculation/rpm_average", rpm_average, Units.RPM);
        Logger.recordOutput("Calculation/rpm_to_meterspersec", rpm_to_meterspersec, Units.MetersPerSecond);
        Logger.recordOutput("Calculation/RobotSpeedVector", robotSpeedVector.getNorm(), Units.MetersPerSecond);
